...
module hvsync_gen
        (clk, reset, hsync, vsync, disp_on, hpos, vpos);
    ...
    output disp_on; // indicated when displayable area active

    // Position Counters-- must be big enough to reach necessary number of clock edges
    output [H_BITS:0] hpos;
    output [V_BITS:0] vpos;

    // Signal position returns
    wire hret = (hpos == H_MAX) || reset;
    wire vret = ((vpos == V_MAX) & hret) || reset;
    ...
    // Usable Area
    assign disp_on = (hpos < H_DISP) && (vpos < V_DISP);
